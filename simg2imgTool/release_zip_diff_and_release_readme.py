...
def set_dev_info():
    ...
    if os.path.exists(osapp_zip_name):                                           #获取当前集成包的版本号：app_config_version_new
        ...
        app_config_version_new=str(osapp_zip_name).split('-')[2].replace(' ','')+'-'+str(osapp_zip_name).split('-')[3].replace(' ','').replace('.zip','')
...
def getappzip_diff(outdir,tardir):
    ...
    if os.path.exists(release_app_zip_name):
        ...
        app_config_version_old=str(release_app_zip_name).split('-')[2].replace(' ','')+'-'+str(osapp_zip_name).split('-')[3].replace(' ','').replace('.zip','')
                ...
                if os.path.exists(appversionzipdir+'/'+str(apk)+'/'+str(app_config[apk])+'/'+'Android.mk'):
                    ...
                    if apkname==(str(apk)+'_'+str(app_config[apk])):
                        ...
                        if os.path.exists(appversionzipdir+'/'+str(apk)+'/'+str(app_config[apk])+'/'+'Readme.txt'):   
                            ...
                            #Settings_Group
                            if str(apk) =='settings_sprd6' or str(apk)=='talpacollect'   or str(apk)=='SetupWizard_talpa' or str(apk)=='OTA' or str(apk)=='TalpaOTA' or str(apk)=='smartkey' or str(apk)=='Talpa_clock' or str(apk)=='talpaConfig' or str(apk)=='talpa_deskclock' or str(apk)=='SetupWizard_sprd' or str(apk)=='settingsProvider_sprd6' or str(apk)=='OS_PluginService' or str(apk)=='settings_mtk'or str(apk)=='settingsProvider_mtk':
                                Settings_Group_app_readme_file.append(readme_files)
							#Telephone_Group
                            if str(apk) =='Dialtact' or str(apk)=='ContactsProvider_talpa' or str(apk)=='TelephonyProvider6' or str(apk)=='talpa_ContaCts':
							...
							#Launcher_Group
                            if str(apk) =='launcher' or str(apk)=='ChangeWallpaper' or str(apk)=='TalpaTheme' or str(apk)=='SPRC_FMRadio' or str(apk)=='FmWidget' or str(apk)=='MTK_FMRadio':
                                Launcher_Group_app_readme_file.append(readme_files)
							#SystemUI_Group
                            if str(apk) =='SystemUI6' or str(apk)=='SystemUI7':
    ...
    if len (group_file)>0:
        for read_file in group_file:
            ...
            if os.path.exists(readme_file):
                file_num = file_num + 1
                num = file_num
                charset =  os.popen('file -i '+readme_file).read()
                print ('file name    is: '+read_file)
                print ('1.file charset is: '+str(charset))
                charset=charset.split('=')[1].replace(' ','').replace('\n','').replace('\r','').replace('\t','')
                print ('2.file charset is: '+str(charset))
                if charset ==('iso-8859-1'):
                    print ('iso-8859-1 file: '+read_file)
                    ChangeEncode(readme_file,"gbk","utf-8")
                    print ('ChangeEncode OK')
                    #open(file).read().decode("gbk")
                    #readme_open_lines = open(readme_file).read().decode("gbk")
