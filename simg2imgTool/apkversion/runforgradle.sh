...
if [ ! -d "$WORKSPACE" ];then
	...
	APKVERSION=$(find $WORKSPACE/code/$JOB_NAME/$JOB_NAME/src/ -name AndroidManifest.xml -exec grep "android:versionName=*" {} \;)
...
if [ $BUILD == 'True' ];then
	...
	if [ "$checktag"!="" -a "$firstline"!="" ];then
		...
		then
			...
			git pull
			git checkout $gittag;
			APKVERcheckSION=$(find $WORKSPACE/code/$JOB_NAME/$JOB_NAME/src/ -name AndroidManifest.xml -exec grep "android:versionName=*" {} \;)
			...
			APKVERSIONCODE=$(find $WORKSPACE/code/$JOB_NAME/$JOB_NAME/src/ -name AndroidManifest.xml -exec grep "android:versionCode=*" {} \;)
