...
def set_dev_info():
    ...
    if osapp_zip_name =='None':
        osapp_zip_name=findlastzipfile(out_osapp_zip_dir+'/'+PRODUCTNAME_OLD)
    else:
                            ...
                            if os.path.exists(outzipfile+'/'+apk+'/'+apkm+'.apk'):
                                ...
                                if "TARGET_OUT_APPS_PRIVILEGED" in paths:
                                    ...
                                    if os.path.exists(outzipfile+'/'+apk+'/'+lib):
                                        ...
                                        os.system('sudo chmod -R 0755 '+package_dir + '/priv-app/'+apk+'/'+lib)
                                    ...
                                    if os.path.exists(outzipfile+'/'+apk+'/'+'fm.conf'):
                                        ...
                                        APKSIZE=getfilesize(outzipfile+'/'+apk+'/'+'fm.conf')+APKSIZE
                                else:
...
def unpack_repack_img():
    ...
    if os.path.exists(img_dir+'/system')or os.path.exists(img_dir+'/system.back.img'):
        print ('...umount '+str(img_dir)+'system')
        os.system('sudo umount '+img_dir+'system')
        os.system('sudo rm -rf '+img_dir+'system')
        os.system('sudo rm -rf '+ img_dir +'system.back.img')
    ...
    if os.path.exists(SCRIPT_PATH+'/tools/'+CHIP+'/'+PRODUCTNAME+'/simg2img') or os.path.exists(SCRIPT_PATH+'/tools/'+'/simg2img'):
        ...
        if str(outs)!='0':
            os.system('sudo cp -raf '+img_dir+'system.img '+img_dir+'system.back.img')
            os.system('sudo mount -o loop -t ext4 '+img_dir+'system.back.img '+img_dir+'system')
    ...
    os.system('sudo sed -i "s/ro.build.display.id=.*/ro.build.display.id='+PRODUCTNAME_OLD+'-'+ appversion+'/g" '+img_dir + '/system/build.prop')
    if CHIP == 'MTK':
        os.system('sudo sed -i "s/ro.itel.version.release=.*/ro.itel.version.release='+PRODUCTNAME_OLD+'-'+ appversion+'/g" '+img_dir + '/system/build.prop')
    else:
        os.system('sudo sed -i "s/ro.build.display.spid.customer=.*/ro.build.display.spid.customer='+PRODUCTNAME_OLD+'-'+ appversion+'/g" '+img_dir + '/system/build.prop')
    ...
    if CHIP == 'MTK':
        ...
        IMG_SIZE='2800000000'
    if PRODUCTNAME == 'P41N':
        IMG_SIZE=IMG_SIZE+APKSIZE+1000000L
        IMG_SIZE=str(IMG_SIZE)
    IMG_SIZE=str(IMG_SIZE)
    ...
    if os.path.exists(SCRIPT_PATH+'/tools/'+CHIP+'/'+PRODUCTNAME+'/make_ext4fs') or os.path.exists(SCRIPT_PATH+'/tools/'+'/make_ext4fs'):
        ...
        os.system('sudo '+make_ext4fstooldir+'/make_ext4fs -T -1 -S ' + SCRIPT_PATH+'/tools/'+CHIP+'/'+PRODUCTNAME + '/file_contexts -l ' + IMG_SIZE + ' -a system '+outsystemimgrepkg_dir_new+'/system-'+PRODUCTNAME+'-'+ appversion+'.img '+img_dir+'/system')
    else:
    ...
    print ('...out_system_path:'+str(outsystemimgrepkg_dir_new)+str('/system-'+PRODUCTNAME+'-'+ appversion+'.img '))
...
def extract_repack_otatarget_zip():
    ...
    os.system('sudo sed -i "s/ro.build.display.id=.*/ro.build.display.id='+PRODUCTNAME_OLD+'-'+ appversion+'/g" '+otatarget_zipdir + '/target/SYSTEM/build.prop')
    if CHIP == 'MTK':
        os.system('sudo sed -i "s/ro.itel.version.release=.*/ro.itel.version.release='+PRODUCTNAME_OLD+'-'+ appversion+'/g" '+otatarget_zipdir + '/target/SYSTEM/build.prop')
    else:
        os.system('sudo sed -i "s/ro.build.display.spid.customer=.*/ro.build.display.spid.customer='+PRODUCTNAME_OLD+'-'+ appversion+'/g" '+otatarget_zipdir + '/target/SYSTEM/build.prop')
...
def to_diff(file1,file2,PRODUCTotapackage_target_file_diff):
    ...
    os.system('sudo chmod 777 ' + ota_from_target_filestool)
    if CHIP==('MTK'):
        cmd = ota_from_target_filestool + ' -k '+SIGN_KEYTOOL+SIGN_KEY+' -s '+mt_ota_from_target_filestool+' -i ' + file2 + ' ' + file1 + ' ' + PRODUCTotapackage_target_file_diff + '/' + os.path.basename(file1)[:-4].replace('_target','') + '_' + time2 + '.zip'
    else:
        cmd = ota_from_target_filestool + ' -k '+SIGN_KEYTOOL+SIGN_KEY+' -i ' + file2 + ' ' + file1 + ' ' + PRODUCTotapackage_target_file_diff + '/' + os.path.basename(file1)[:-4].replace('_target','') + '_' + time2 + '.zip'
    print ('cmd:'+str(cmd))
    p = subprocess.Popen(cmd, stdin=subprocess.PIPE, stdout=subprocess.PIPE, shell=True)
    if input_sign_key != None:
        p.communicate(input_sign_key)
