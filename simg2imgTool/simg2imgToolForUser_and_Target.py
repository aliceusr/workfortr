...
# python ${simg2imgTool}/simg2imgTool.py ${product_chip} ${product_name} ${osapp_zip_name}   ${targetBackup}   
#                                              1                 2                3                               
...
def set_dev_info():
    ...
    unpack_repack_img()
	#extract_repack_otatarget_zip() #备份target包，生成OTA包 chb  add  for target
                    ...
                    if apkm==apk:
                        ...
                        if os.path.exists(outzipfile+'/'+apkm+'/'+apk_src_files):
                            ...
                            if os.path.exists(outzipfile+'/'+apk+'/'+apkm+'.apk'):
                                print('chb  exists'+outzipfile+'/'+apk+'/'+apkm+'.apk')
                                paths=''
                                ...
                                print('chb -not exists'+outzipfile+'/'+apk+'/'+apkm+'.apk')
...
def unpack_repack_img():
    ...
    os.system('sudo sed -i "s/ro.build.display.id=.*/ro.build.display.id='+PRODUCTNAME_OLD+'-'+file_time+'-'+ appversion+'/g" '+img_dir + '/system/build.prop')
    #chb add for user beta
    os.system('sudo sed  -i "s/ro.itel.version.release=.*/&_beta_'+appversion+'/g" '+img_dir + '/system/build.prop')
...
def extract_repack_otatarget_zip():
    ...
    print (str(fctimes))
    ctimes=max(ctimes)
    target_file_name = str(fctimes[ctimes]) #chb add for target
